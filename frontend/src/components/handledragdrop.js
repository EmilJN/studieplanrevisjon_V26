...
} from "../utils/checkforconflicts.js";
...
export const handleDragEnd = async ({
  ...
  if (source.droppableId.startsWith("semester-")) {
    sourceSemesterNumber = parseInt(
      source.droppableId.substring("semester-".length),
    );
    console.log("sourceSemesterNumber:", sourceSemesterNumber);
  }

  if (destination.droppableId.startsWith("semester-")) {
    destSemesterNumber = parseInt(
      destination.droppableId.substring("semester-".length),
    );
  ...
  if (
    ...
    try {
      ...
      const courseExists = destSemester.semester_courses.some(
      ...
      const { termConflictsHandled, newTermConflicts } =
        await handleTermConflicts(courseToAdd, destSemester, "add");
      ...
      setSuccessMessage(
        `Added ${courseToAdd.courseCode} to semester ${destSemester.semester_number}`,
      );
    } catch (error) {
  ...
  if (
    ...
    if (courseExistsInDestination) {
      setErrorMessage(
        `The course ${courseToMove.courseCode} already exists in semester ${destSemester.semester_number}.`,
      );
      return;
    }
    ...
    try {
      const { termConflictsHandled, newTermConflicts } =
        await handleTermConflicts(courseToMove, destSemester, "move");
      if (!termConflictsHandled) return;
      updateConfirmedConflicts(newTermConflicts, setConfirmedConflicts);

      const updatedSourceSemester = {
        ...sourceSemester,
        semester_courses: sourceSemester.semester_courses.filter(
