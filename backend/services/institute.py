from app import db
from app.models import Institute


class InstituteService:
    def __init__(self, db_session=None):
        self.db = db_session or db.session

    def create_institute(self, name):
        try:
            new_institute = Institute(name=name)
            self.db.add(new_institute)
            self.db.commit()
            return new_institute

        except Exception as e:
            self.db.rollback()
            raise RuntimeError(
                f"Failed to create institute: {str(e)}"
            )

    def get_all_institutes(self):
        try:
            return self.db.query(Institute).all()
        except Exception as e:
            raise RuntimeError(
                f"Failed to fetch institutes: {str(e)}"
            )

    def get_institute_by_id(self, institute_id):
        try:
            return (
                self.db.query(Institute)
                .filter(Institute.id == institute_id)
                .first()
            )

        except Exception as e:
            raise RuntimeError(
                f"Failed to fetch institute: {str(e)}"
            )

    def update_institute(self, institute_id, name):
        try:
            institute = self.get_institute_by_id(
                institute_id
            )
            if not institute:
                raise ValueError(
                    "Institute not found"
                )
            institute.name = name
            self.db.commit()
            return institute

        except ValueError:
            raise
        except Exception as e:
            self.db.rollback()
            raise RuntimeError(
                f"Failed to update institute: {str(e)}"
            )

    def delete_institute(self, institute_id):
        try:
            institute = (
                self.db.query(Institute)
                .get(institute_id)
            )

            if not institute:
                raise ValueError(
                    f"Institute with id "
                    f"{institute_id} does not exist."
                )

            self.db.delete(institute)
            self.db.commit()
            return True
        except ValueError:
            raise

        except Exception as e:
            self.db.rollback()
            raise RuntimeError(
                f"Failed to delete institute "
                f"{institute_id}: {str(e)}"
            )